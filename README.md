...
Container bridge for **InvestorClaw 4.1.20** — the InvestorClaw application
service that pairs `ic-engine` (Python, FINOS CDM 5.x portfolio analysis)
with `mnemos-rs` (Rust, sqlite-backed memory).

This repo owns:
- The `ic-engine:4.1.20-cpu` container Dockerfile (Python 3.12 + uv +
...
├── Dockerfile             # Apache 2.0 — ic-engine:4.1.20-cpu image build
